...
- **换版面包**：内置三份（`/shudlayout arena|olympia|clean`），也可以手工改 JSON，
  约 1 秒内所有画面自动跟随。
...
包装分两层。**常驻信息层**是贴边的角标、计时、选手条、信息条、公告、安全框、自由文本；
**数据看板层**是八种板：数据对比、排行榜、趋势曲线、关键指标、阵容卡、局分板、事件时间轴、对阵卡。

九种数据看板（含自由文本可绑定的路径）读的是**同一个数据形状**：一张有名字的表，
每行是一个「标签 + 数值 + 属于哪一方」。所以：

```jsonc
// 状态里的一块看板
{ "key": "compare", "title": "数据对比",
  "rows": [
    { "key": "击杀", "label": "击杀", "side": "home", "value": 42 },
    { "key": "击杀", "label": "击杀", "side": "away", "value": 37 } ] }
```

`key` 相同的两行会被画成一根从中轴往两边长的条 —— 这就是「对抗」而不是「两份报告」。
加第十种看板不需要改协议，也不需要加命令：元素用 `options.board` 指向哪张表，
渲染器就画哪张表；表里没有行，元素整块收掉，而不是在画面上留一个空框。

写入只有一条路径：`/stevehud stat <看板> <数值> <名称…> [@方位]`。

每个元素的 `options` 是一个**开放**的键值表：渲染器认识的键有中文标签和合适的控件，
不认识的键原样保留在文档里。所以「先在编辑器里存一个设置、渲染器以后再支持」是可行的顺序，
而不是一个会丢数据的死胡同。

### 「不遮挡观看」是被证伪的，不是一句口号

画面窗口定义为画布中央 60% × 68%，那是观众真正在看的地方。规则有三条：

1. 没有声明 `options.window = "allow"` 的元素，压进画面窗口超过 2% 就是**故障**；
2. 一个场景整体占用画面窗口超过 3% 是故障，除非该场景里有已声明的中场大卡（此时上限 70%，
   因为中场大卡本来就是要在几秒内接管整个画面）；
3. 文字永远先缩字号，缩到下限仍溢出才算故障 —— 这是「英文队名显示不全」那类问题的根治办法。

同一套规则有两个执行者：编辑器的画布右下角实时显示遮挡预算，`tools/overlay-audit.mjs`
在打包检查里跑，任何一条不满足就非零退出。
...
| 编辑器 UI 规范 | **打版台（dark console）+ 单一强调色**：深色底、1px 分隔、层次只靠边框与中性灰阶；强调色取自被编辑的版面包，所以工具被它编辑的东西染色 |
| 编辑器的页面布局 | **可改**：左右两栏可拖宽窄、双击折叠，状态存 localStorage —— 同一套工具要同时开在 1080p 和 4K 上，一个固定宽度不可能两边都合适 |
| 数据看板的数据形状 | **一种**：`{key,title,rows:[{key,label,side,value,display,…}]}`。加看板不改协议、不加命令 |
| 「不遮挡观看」 | **可执行的三条预算**：单个元素压进画面窗口 ≤2%（除非声明 `window=allow`）、整场景 ≤3%（含声明的中场大卡时 ≤70%）、文字缩到下限不得仍溢出。编辑器实时显示，审计脚本在打包检查里强制执行 |
...
| 版面文档模型 + 三份内置版面包（arena / olympia / clean）+ 元素级开放 `options` | **完成** |
| 八个新数据看板类型 + 看板数据模型（`BroadcastState.Board`） | **完成** |
| 游戏内 HUD：由文档驱动、调色板、每元素样式与动画、含看板绘制 | **完成**（四版本编译通过） |
| 本机图形服务：HTTP + SSE + 静态页 + REST | **完成** |
| 网页包装：16 种元素、主题令牌、`--u` 画布单位、`--k` 文字跟随框、自适应字号 | **完成**（浏览器实测 + 自动化审计） |
| 版面编辑器：可拖拽/可折叠双栏、元素调色板、开放参数编辑器、参考线与实时遮挡预算、预设画廊、实时预览、保存广播 | **完成**（浏览器实测） |
| 版面审计 `tools/overlay-audit.mjs`：遮挡预算 / 文字裁切 / 元素重叠 / 页面报错 | **完成**（6 种组合全通过） |
| 单元测试 | **117 项**（协议 34 + 共享 83），全绿 |
...
```powershell
node toolsoverlay-audit.mjs --all
```

用一个真实的 Edge/Chrome 无头实例加载每一套预设 × 每一个场景，然后**量**出来：

- 每个元素的实际矩形，以及两两之间有没有重叠；
- 画面窗口与整幅画面的遮挡比例，对照上面那三条预算；
- 哪些文本触发了自适应缩放、缩到了多少、有没有缩到下限还在溢出；
- 页面报了什么错、哪些请求 404。

它存在的理由是「看着挺好」不是一个能在下一套预设上依然成立的检查。退出码可直接进 CI；
`--json` 给机器读，默认输出给人读。

要看画面本身，用 `tools/overlay-shot.mjs`。包装页在 OBS 里是叠在游戏上的、背景透明，
所以**直接截图几乎证明不了什么** —— 一块在纯黑底上读起来很清楚的板，压到亮天空上可能完全糊掉。
它因此会注入一层"替身画面"（亮天空、中间调地形、暗地面、一暖一冷两处高光），再把包装拍在上面：
...
它在拍之前会把入场动画收掉。这不是美化：入场关键帧从透明度 0 开始，抢在动画中间拍出来的
是一张"整套包都是空的"的图 —— 非常有说服力，而且完全是假的。

补上另外半边的是 `tools/web-probe.mjs`：在真实页面里跑一段你给的表达式，打印返回值，
并把页面抛的异常和报错一并带出来（缺 favicon 不算）。审计问的是「这套包画得出来吗」，
它问的是「这个页面还能用吗」：
